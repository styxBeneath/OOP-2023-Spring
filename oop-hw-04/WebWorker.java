...
public class WebWorker extends Thread {
	...
	private void download() {
		...
		try {
			...
			successfulDownload(contents.length(), System.currentTimeMillis() - startTime);
		}
	...
	private void successfulDownload(int size, long elapsedTimeMillis) {
		StringBuffer buff = new StringBuffer();
		SimpleDateFormat format = new SimpleDateFormat("hh:mm:ss");
		format.format(new Date(System.currentTimeMillis()), buff, new FieldPosition(0));
		buff.append(" ").append(elapsedTimeMillis).append("ms ").append(size).append(" bytes");
